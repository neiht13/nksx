...
var QRCodeImpl = require('./QRCode');

var ErrorCorrectLevel = require('./ErrorCorrectLevel'); // TODO: pull this off of the QRCode class type so it matches.
...
var QRCodeCanvas =
    ...
    return;
                    ...
                    var radius = 0.5
                    var img = this.state && this.state.imgLoaded && this._image;
                    ctx.beginPath();
                            ...
                            if (cell) {
                                switch (qrStyle) {
                                    ...
                                    case "vnpt":
                                        ...
                                        ctx.arc(X + radius + margin, Y + radius + margin, radius, 0, Math.PI * 2, true);
                                        if (row[X + 1]) {
                                            ctx.fillRect(X + radius + margin, Y + margin, 1, 1);
                                        }
                                        if ((Y + 1) < cells.length && cells[Y + 1][X]) {
                                            ctx.fillRect(X + margin, Y + radius + margin, 1, 1);
                                        }
                                        break;
                                }
                            }
